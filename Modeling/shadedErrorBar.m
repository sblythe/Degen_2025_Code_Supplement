function varargout=shadedErrorBar(x,y,errBar,color,meanWidth,errorShade,varargin)
...
params.addParameter('patchSaturation', errorShade, @(x) isnumeric(x) && x>=0 && x<=1); % NOTE usually 0.2, 0.4 for onset distributions
...
H = makePlot(x,y,errBar,color,meanWidth,lineProps,transparent,patchSaturation);
...
function H = makePlot(x,y,errBar,color,meanWidth,lineProps,transparent,patchSaturation)
    ...
    if hostName == 'MATLAB'
      H.mainLine=plot(x,y,'linewidth',meanWidth,'color',color); % NOTE linewidth usually 1.5, 1 for fig 2 and 3
